...
export default function AdminReelsPage() {
      ...
      if (!error) {
        ...
        reader.onloadend = () => {
          if (reader.result) {
            ...
            showToast('✨ Thumbnail loaded from file!');
          }
        };
  ...
  const handleOpenEditModal = (reel: ReelItem) => {
    ...
    setThumbnailUrl(reel.thumbnailUrl || reel.previewUrl || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const reelPayload: ReelItem = {
      ...
      previewUrl: thumbnailUrl.trim() || undefined,
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredReels.map((reel) => (
            <div
              key={reel.id}
              className="bg-zinc-950 border border-zinc-800/80 rounded-2xl p-5 flex flex-col justify-between hover:border-primary/40 transition-all group overflow-hidden"
            >
              <div>
                {/* Thumbnail Preview Banner if available */}
                {(reel.thumbnailUrl || reel.previewUrl) && (
                  <div className="h-36 -mx-5 -mt-5 mb-4 relative overflow-hidden bg-zinc-900 border-b border-zinc-800/80">
                    <img
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {/* Thumbnail / Cover Image Upload & Input */}
                <div>
                  <label className="block text-xs font-semibold text-zinc-300 uppercase tracking-wider mb-2">
                    Reel Thumbnail / Cover Image
                  </label>

                  {thumbnailUrl ? (
                    <div className="relative rounded-xl overflow-hidden border border-zinc-800 bg-zinc-900/90 p-2.5 mb-3 flex items-center gap-4">
                      <img
                        src={thumbnailUrl}
                        alt="Thumbnail preview"
                        className="w-14 h-20 object-cover rounded-lg border border-zinc-800 shrink-0"
                      />
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-zinc-300 truncate font-mono">{thumbnailUrl}</p>
                        <p className="text-[11px] text-primary mt-1 font-medium flex items-center gap-1">
                          <CheckCircle2 className="w-3 h-3" /> Cover Image Ready
                        </p>
                      </div>
                      <button
                        type="button"
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    <input
                      ...
                      className="flex-1 px-4 py-2.5 rounded-xl bg-zinc-900 border border-zinc-800 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-primary/50"
                    />

                    <label className="px-4 py-2.5 rounded-xl bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 hover:border-zinc-700 text-zinc-300 hover:text-white text-xs font-semibold flex items-center justify-center gap-2 cursor-pointer transition-colors shrink-0">
                      {isUploadingThumb ? (
                        <>
